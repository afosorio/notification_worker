...
class ReplayNotificationServiceTest {
    ...
    void publishesOnlyAfterTheConditionalUpdateSucceeds() {
        when(repository.findById("EVT003")).thenReturn(Optional.ofNullable(null));
    ...
    void rejectsAnEventThatIsNotFailed() {
        when(repository.findById("EVT001")).thenReturn(Optional.of(mock(com.cobre.notifications.domain.NotificationEvent.class)));
    ...
    void publishesWhenFailedStateIsTransitionedAtomically() {
        when(repository.findById("EVT003")).thenReturn(Optional.of(mock(com.cobre.notifications.domain.NotificationEvent.class)));
        when(repository.replayIfFailed("EVT003")).thenReturn(true);
